...
unsigned char *AES::EncryptECB(const unsigned char *in, unsigned int inLen,
  ...
  for (unsigned int i = 0; i < numBlocks; ++i) {
    EncryptBlock(in + i * blockBytesLen, out + i * blockBytesLen,
                 roundKeys.data());
  }
  return out;
}
std::vector<unsigned char> AES::DecryptECB(const unsigned char *in,
                                           unsigned int inLen,
                                           const unsigned char *key) {
  CheckLength(inLen);

  std::vector<unsigned char> out(inLen);
  std::unique_ptr<unsigned char[]> roundKeys(
      new unsigned char[4 * Nb * (Nr + 1)]);
...
std::vector<unsigned char> AES::EncryptCBC(const unsigned char *in,
                                           unsigned int inLen,
                                           const unsigned char *key,
...
void AES::ShiftRow(unsigned char state[4][Nb],
                   unsigned int row.unsigned int shift) {
